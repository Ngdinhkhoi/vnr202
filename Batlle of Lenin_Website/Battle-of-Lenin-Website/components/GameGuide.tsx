...
const StepCard: React.FC<{
  ...
  <div className="relative group p-6 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-300 hover:-translate-y-1">
    <div
      className={`absolute -top-3 -left-3 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm bg-gray-900 border-2 ${
        color === "gold"
    ...
    <h4
      className={`text-lg font-bold mb-2 ${
        color === "gold" ? "text-brand-gold" : "text-emerald-400"
      }`}
    >
...
const GameGuide: React.FC = () => {
  ...
  const accentColor =
    activeTab === "quiz" ? "text-brand-gold" : "text-emerald-400";
  const bgAccent = activeTab === "quiz" ? "bg-brand-gold" : "bg-emerald-500";
  const shadowAccent =
    activeTab === "quiz" ? "shadow-brand-gold/20" : "shadow-emerald-500/20";
  ...
  return (
    <section className="relative w-full py-16 px-4 md:px-8 max-w-6xl mx-auto">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full pointer-events-none overflow-hidden">
        <div
          className={`absolute top-20 left-1/4 w-96 h-96 rounded-full blur-[100px] opacity-20 transition-colors duration-700 ${bgAccent}`}
        ></div>
      </div>
      ...
      <div className="relative z-10 text-center mb-12">
        <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight text-white mb-4">
          Hướng Dẫn{" "}
          <span className={`transition-colors duration-500 ${accentColor}`}>
            Luật Chơi
          </span>
        </h2>
        <p className="text-gray-400 max-w-2xl mx-auto">
          Nắm vững quy tắc để chinh phục bảng xếp hạng VNR202.
        </p>
      </div>
      ...
      <div className="relative z-10 flex justify-center mb-16">
        <div className="p-1.5 bg-white/10 backdrop-blur-md rounded-full border border-white/10 inline-flex relative shadow-xl">
          ...
          <div
            className={`absolute top-1.5 bottom-1.5 rounded-full transition-all duration-500 ease-out shadow-lg ${bgAccent} ${
              activeTab === "quiz"
                ? "left-1.5 w-[140px]"
                : "left-[146px] w-[150px]"
            }`}
          ...
          <button
            ...
            className={`relative w-[140px] py-2.5 rounded-full text-sm font-bold uppercase tracking-wider transition-colors duration-300 z-10 ${
              activeTab === "quiz"
                ? "text-gray-900"
          ...
          {/* <button
            ...
            className={`relative w-[150px] py-2.5 rounded-full text-sm font-bold uppercase tracking-wider transition-colors duration-300 z-10 ${
              activeTab === "matching"
                ? "text-gray-900"
                : "text-gray-400 hover:text-white"
            }`}
      ...
      <div className="relative z-10 animate-fade-in-up">
        <div
          className={`rounded-3xl border border-white/10 bg-[#121218]/80 backdrop-blur-xl p-8 md:p-12 shadow-2xl transition-all duration-500 ${shadowAccent}`}
        >
          {/* Intro */}
          <div className="text-center mb-10">
            <span
              className={`inline-block px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest bg-white/10 mb-4 ${accentColor}`}
            >
              {activeTab === "quiz"
                ? "Kiến thức & Phản xạ"
                : "Trí nhớ & Tư duy"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {(activeTab === "quiz" ? quizSteps : matchingSteps).map(
              (step, idx) => (
                <StepCard
                  key={idx}
                  step={step}
          ...
          <div className="border-t border-white/10 pt-8">
            <h4 className="text-center text-sm font-bold uppercase text-gray-400 tracking-widest mb-6">
              Lưu ý quan trọng
            </h4>
            <div className="flex flex-wrap justify-center gap-4 md:gap-8">
              {playHighlights[activeTab].map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 text-sm md:text-base text-gray-300 bg-white/5 px-5 py-2 rounded-lg border border-white/5"
                >
