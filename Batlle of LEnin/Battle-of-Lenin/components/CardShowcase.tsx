...
const QUIZ_STAGE_TRACKS: Record<
  QuizStage,
...
const CardShowcase: React.FC<CardShowcaseProps> = ({
  ...
  >({});
  const [playerResults, setPlayerResults] = useState<Record<
    number,
    QuizResult
  > | null>(null);
  ...
  const finalizeQuiz = useCallback(() => {
    ...
    const rawPoints = Math.max(
      0,
      Math.round((remaining / ANSWER_TIME_LIMIT) * MAX_POINTS)
    );
  ...
  const timeFraction =
    answerTimeLeft !== null
      ? Math.max(0, answerTimeLeft) / ANSWER_TIME_LIMIT
      : 0;
  const projectedPoints = Math.max(
    0,
    Math.round(timeFraction * MAX_POINTS) - helpPenalty
  );
            ...
            <div className="w-full max-w-4xl bg-[#080808] border border-white/10 rounded-3xl shadow-2xl text-white p-6 space-y-6 relative">
                ...
                {card.imageSrc && card.imageSrc !== "" ? (
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.currentTarget.src = COVER_IMAGE;
                      e.currentTarget.alt = "Không có hình ảnh";
                    }}
                    draggable={false}
                  />
                ) : (
              ...
              <div className="flex flex-col items-stretch sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="flex-1 text-right text-xs text-gray-300 space-y-1">
                  ...
                  {helpPenalty > 0 && (
                    <p>
                      Trả lời đúng: điểm thưởng bị trừ; trả lời sai: bị trừ
                      thẳng {helpPenalty}đ vào tổng điểm.
                    </p>
                  )}
                  {!quizFinished && showQuestionForm && (
                    <>
                      {!canAffordAnswerBot && (
                        <p>
                          Yêu cầu &gt; 70 điểm để dùng bot gợi ý (hiện có{" "}
                          {highestPlayerScore}đ).
                        </p>
                      )}
                      {!canAffordEliminateBot && (
                        <p>
                          Yêu cầu &gt; 40 điểm để loại đáp án (hiện có{" "}
                          {highestPlayerScore}đ).
                        </p>
                      )}
                    </>
                    ...
                    >
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        {quiz.answers.map((answer, index) => {
                          ...
                          const isCorrectChoice = correctAnswers.has(
                            answer.content
                          );
                          const isEliminated = eliminatedOptions.has(
                            answer.content
                          );
                          ...
                          return (
                            ...
                            >
                              <input
                                ...
                                checked={active}
                                onChange={() =>
                                  handleSelect(player.id, answer.content)
                                }
                                disabled={quizFinished || isEliminated}
                                className="mt-1 accent-red-500"
                              />
                              <span>
                                <span className="text-xs uppercase tracking-widest text-red-300 pr-2">
                                  {letter}
                                </span>
                                {answer.content}
                              </span>
